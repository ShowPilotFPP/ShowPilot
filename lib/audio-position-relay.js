...
function connect() {
  ...
  sock.on('close', () => {
    ws = null;
    if (isRunning) {
      console.log('[position-relay] daemon WebSocket closed, reconnecting in 3s');
      reconnectTimer = setTimeout(connect, 3000);
    }
  });

  sock.on('error', (err) => {
    // Don't log ECONNREFUSED spam — daemon may not be running yet
    if (err.code !== 'ECONNREFUSED') {
      console.warn('[position-relay] WebSocket error:', err.message);
    }
    ws = null;
    if (isRunning) {
