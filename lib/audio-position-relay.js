...
function connect() {
  ...
  try { WebSocket = require('ws'); } catch (_) {
    try { WebSocket = require(require.resolve('ws', { paths: ['/opt/showpilot/node_modules', __dirname + '/../node_modules'] })); } catch (_2) {
      console.warn('[position-relay] ws module not available — position relay disabled');
      return;
    }
  }
  ...
  sock.on('message', (data) => {
    try {
      ...
      if (msg.type === 'position' && io) {
        ...
        // Broadcast sync point — viewers use this to coordinate play start.
        // All devices receiving the same syncPoint event seek to the same
        // position and schedule play at the same server-time moment.
